...
def login_2fa():
    ...
    secret = qrcode.make(pyotp.totp.TOTP(s).provisioning_uri())
    buffer = io.BytesIO()
    secret.save(buffer, format="PNG")
    img_str = 'data:image/png;base64, ' + base64.b64encode(buffer.getvalue()).decode("utf-8")
    return render_template("login_2fa.html", secret=img_str, code=s)
